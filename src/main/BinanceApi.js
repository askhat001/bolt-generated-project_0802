const CONFIG = require('../../config/config');
    ...
    const StatisticalArbitrage = require('./StatisticalArbitrage'); // Import
    ...
    const BinanceApi = {
        ...
        createDepthWSCallback(cb) {
            ...
            return (ticker, depth, context) => {
                if (context) {
                    // Get the mid-price and update the price history.
                    const midPrice = CalculationNode.getMidPrice(depth);
                    if (midPrice !== null) {
                        StatisticalArbitrage.updatePriceHistory(ticker, midPrice);
                    }
                    cb(ticker);
                }
            }
